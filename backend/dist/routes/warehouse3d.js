...
import { bump, publishForkliftState } from '../lib/bus.js';
...
const forkliftControllerLeases = new Map();
function clientIdFrom(req, required = false) {
    const clientId = String(req.get('X-Client-Id') ?? '').trim();
    if (!clientId) {
        if (required)
            throw httpError(400, 'ต้องระบุ X-Client-Id เพื่อควบคุมรถโฟล์คลิฟต์', 'client_id_required');
        return null;
    }
...
function activeForkliftLease(warehouseId, now = Date.now()) {
...
function forkliftControllerStatus(warehouseId, requesterId, now = Date.now()) {
...
function claimForkliftControl(warehouseId, clientId, now = Date.now()) {
    const lease = activeForkliftLease(warehouseId, now);
...
function sendForkliftConflict(res, warehouseId, requesterId) {
...
const parseBoxTypeDimensions = (value) => {
    if (!value)
        return null;
...
const rackJson = (rack, rackSlots, boxCountBySlot = new Map()) => ({
    id: rack.id,
...
warehouse3dRouter.get('/', asyncHandler(async (req, res) => {
    ...
    const configuredGateNumbers = Array.isArray(warehouse?.gates)
        ? warehouse.gates.map((gate) => Number(gate)).filter(Number.isFinite)
        : [];
    ...
    res.json({
        ...
        forkliftPosition: warehouse?.data && typeof warehouse.data === 'object'
            ? warehouse.data.forkliftPosition ?? null
            : null,
        ...
        boxes: boxRows.map((box) => {
            ...
            return {
                ...
                slotId: box.slotId,
                status: box.status,
                slotSide: box.data?.slotSide === 'right' ? 'right' : 'left',
...
warehouse3dRouter.post('/forklift/control/claim', requirePermission('master.manage'), asyncHandler(async (req, res) => {
    ...
    if (!warehouse)
        throw httpError(404, 'ไม่พบคลัง', 'warehouse_not_found');
    if (!claimForkliftControl(warehouseId, clientId))
        return sendForkliftConflict(res, warehouseId, clientId);
    res.json({ ok: true, forkliftController: forkliftControllerStatus(warehouseId, clientId) });
}));
warehouse3dRouter.post('/forklift/control/release', requirePermission('master.manage'), asyncHandler(async (req, res) => {
    const { warehouseId } = forkliftControlSchema.parse(req.body);
    const clientId = clientIdFrom(req, true);
    const db = getDb();
    const [warehouse] = await db.select({ id: warehouses.id }).from(warehouses).where(eq(warehouses.id, warehouseId));
...
warehouse3dRouter.put('/forklift', requirePermission('master.manage'), asyncHandler(async (req, res) => {
...
warehouse3dRouter.put('/racks/:id', requirePermission('master.manage'), asyncHandler(async (req, res) => {
    const input = rackGeometrySchema.parse(req.body);
    const db = getDb();
    const [before] = await db.select().from(racks).where(eq(racks.id, req.params.id));
    if (!before)
        throw httpError(404, 'ไม่พบแร็ก', 'rack_not_found');
...
warehouse3dRouter.put('/slots/:id', requirePermission('master.manage'), asyncHandler(async (req, res) => {
    const input = slotGeometrySchema.parse(req.body);
    const db = getDb();
    const [before] = await db.select().from(slots).where(eq(slots.id, req.params.id));
    if (!before)
        throw httpError(404, 'ไม่พบช่องจัดเก็บ', 'slot_not_found');
    const dimensions = input.dimensionsCm ?? {};
...
//# sourceMappingURL=warehouse3d.js.map