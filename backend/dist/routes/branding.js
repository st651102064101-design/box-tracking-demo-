...
brandingRouter.get('/', asyncHandler(async (_req, res) => {
...
brandingRouter.get('/favicon', asyncHandler(async (_req, res) => {
    const rows = await getDb().select().from(config).where(eq(config.id, 1));
...
brandingRouter.put('/', requireAuth, asyncHandler(async (req, res) => {
