...
stateRouter.put('/', requireRole('admin', 'staff'), asyncHandler(async (req, res) => {
    ...
    if (req.user?.role !== 'admin') {
        ...
        const currentAccess = new Map(current.map((e) => [e.id, e.data?.access]));
        for (const [id, raw] of Object.entries(payload.employees ?? {})) {
            const incomingAccess = raw?.access;
