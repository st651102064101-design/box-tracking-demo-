...
async function fetchMessagingProfile(userId) {
    if (!env.line.channelAccessToken || !LINE_USER_ID.test(userId))
        return null;
    try {
...
function configured() {
    return Boolean(env.line.loginChannelId &&
...
lineLinkRouter.post('/unlink', requireAuth, requirePermission('partner.update'), asyncHandler(async (req, res) => {
    ...
    const nextData = { ...(customer.data ?? {}) };
    delete nextData.lineUserId;
...
lineLinkRouter.post('/invitations', requireAuth, requirePermission('partner.update'), asyncHandler(async (req, res) => {
    if (!configured()) {
    ...
    const [customer] = await db
        ...
        .limit(1);
    if (!customer)
        throw httpError(404, 'ไม่พบข้อมูลลูกค้า', 'customer_not_found');
    const token = secret();
...
lineLinkRouter.get('/login', asyncHandler(async (req, res) => {
    if (!configured())
        return res.status(503).send(resultPage('ยังไม่พร้อมใช้งาน', 'ผู้ดูแลระบบยังตั้งค่า LINE Login ไม่ครบ', false));
    ...
    const [invite] = await db
        ...
        .limit(1);
    if (!invite)
        return res.status(404).send(resultPage('ลิงก์ไม่ถูกต้อง', 'ไม่พบลิงก์เชิญนี้ กรุณาขอลิงก์ใหม่จากผู้ดูแลระบบ', false));
    if (invite.consumedAt)
        return res.status(409).send(resultPage('ลิงก์นี้ถูกใช้ไปแล้ว', 'บัญชี LINE ถูกผูกสำเร็จแล้ว ลิงก์เชิญหนึ่งรายการใช้ได้เพียงครั้งเดียว', false));
    if (invite.expiresAt <= new Date())
        return res.status(410).send(resultPage('ลิงก์หมดอายุแล้ว', 'กรุณาขอลิงก์ใหม่จากผู้ดูแลระบบ', false));
    ...
    if (!customer)
        return res.status(404).send(resultPage('ไม่พบลูกค้า', 'ข้อมูลลูกค้านี้ถูกลบหรือปิดใช้งานแล้ว', false));
    if (customer.lineUserId)
        return res.status(409).send(resultPage('ลูกค้านี้ผูก LINE แล้ว', 'ไม่สามารถใช้ QR เดิมเพื่อผูกซ้ำได้ ผู้ดูแลระบบต้องเลือกสร้างลิงก์ใหม่ก่อน', false));
    const state = secret();
    const nonce = secret();
...
lineLinkRouter.get('/callback', asyncHandler(async (req, res) => {
    if (!configured())
        return res.status(503).send(resultPage('ยังไม่พร้อมใช้งาน', 'ผู้ดูแลระบบยังตั้งค่า LINE Login ไม่ครบ', false));
    if (req.query.error)
        return res.status(400).send(resultPage('ยกเลิกการเชื่อมต่อ', 'ยังไม่ได้อนุญาตให้ Box Tracking เข้าถึงโปรไฟล์ LINE', false));
    const parsed = z.object({ code: z.string().min(1), state: z.string().min(20).max(200) }).safeParse(req.query);
    if (!parsed.success)
        return res.status(400).send(resultPage('ลิงก์ไม่ถูกต้อง', 'ไม่พบข้อมูลยืนยันจาก LINE', false));
    const db = getDb();
    const [invite] = await db
        .select({
        ...
        .from(lineLinkInvites)
        .where(and(eq(lineLinkInvites.oauthStateHash, hash(parsed.data.state)), isNull(lineLinkInvites.consumedAt), gt(lineLinkInvites.expiresAt, new Date())))
        .limit(1);
    if (!invite?.nonce || !invite.codeVerifier)
        return res.status(410).send(resultPage('ลิงก์ใช้ไม่ได้แล้ว', 'ลิงก์หมดอายุ ถูกใช้แล้ว หรือไม่ได้เริ่มจากลิงก์เชิญ', false));
    ...
    const [duplicate] = await db.select({ id: customers.id, name: customers.name })
        ...
        .limit(1);
    if (duplicate)
        return res.status(409).send(resultPage('บัญชีนี้ถูกใช้งานแล้ว', `LINE บัญชีนี้ผูกกับลูกค้า ${duplicate.name ?? duplicate.id} อยู่แล้ว`, false));
    const [customer] = await db.select({ id: customers.id, name: customers.name, data: customers.data, lineUserId: customers.lineUserId })
        .from(customers).where(and(eq(customers.id, invite.customerId), isNull(customers.deletedAt))).limit(1);
    if (!customer)
        return res.status(404).send(resultPage('ไม่พบลูกค้า', 'ข้อมูลลูกค้าถูกลบหรือปิดใช้งานแล้ว', false));
    await db.transaction(async (tx) => {
        ...
        if (!claimed.length)
            throw httpError(409, 'ลิงก์นี้ถูกใช้ไปแล้ว', 'line_link_already_used');
        await tx.update(customers).set({
            lineUserId: profile.sub,
            lineDisplayName: profile.name ?? null,
            linePictureUrl: profile.picture ?? null,
            lineLinkedAt: new Date(),
            data: { ...customer.data, lineUserId: profile.sub },
            updatedAt: new Date(),
...
//# sourceMappingURL=lineLink.js.map