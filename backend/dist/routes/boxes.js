...
const OCCUPYING_STATUSES = ['warehouse', 'hold', 'damage'];
const text = (value) => String(value ?? '').trim();
const recordOf = (value) => value && typeof value === 'object' && !Array.isArray(value) ? value : {};
const isReportedFull = (value) => text(recordOf(value).reportedFullAt) !== '';
/**
...
boxesRouter.post('/:tag/putaway', canUpdate, asyncHandler(async (req, res) => {
    ...
    const data = await db.transaction(async (tx) => {
        ...
        if (!['pending', 'warehouse'].includes(box.status)) {
            throw httpError(409, `กล่อง ${box.tag} สถานะ "${box.status}" ไม่สามารถ Putaway หรือย้ายชั้นได้`, 'box_not_putaway_eligible');
        }
        ...
        if (hasExactLocation) {
            [targetSlot] = await tx
                ...
                .where(and(eq(racks.warehouseId, input.wh), eq(racks.zone, input.zone), eq(racks.code, input.rack), eq(slots.shelfCode, input.shelf), eq(slots.slotCode, input.slot)));
            if (!targetSlot) {
                ...
                const [geometry] = await tx
                    ...
                    .limit(1);
                if (geometry)
                    throw httpError(404, 'ไม่พบช่องจัดเก็บที่เลือก', 'slot_not_found');
            }
        }
        if (targetSlot) {
            ...
            const [occupancy] = await tx
                .select({ count: count() })
                .from(boxes)
                .where(and(eq(boxes.slotId, targetSlot.id), inArray(boxes.status, [...OCCUPYING_STATUSES]), ne(boxes.tag, box.tag)));
