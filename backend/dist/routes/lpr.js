...
const expectedRfidReads = new Map([
    ['GATE-1', { plateNumber: '1กข1234', epc: 'BOX-001', seenAt: Date.now() }],
]);
const recentRfidReads = new Map();
 ...
 * dependency-free. */
export function recordRfidObservation(gateId, tags, seenAt = Date.now()) {
...
lprRouter.post('/', asyncHandler(async (req, res) => {
    ...
    if (!gate)
        throw httpError(404, `ไม่พบ DB Gate ${gateNo} สำหรับ LPR camera`, 'lpr_gate_not_found');
    const gateKey = `GATE-${gateNo}`;
    const duplicate = processedEvents.get(payload.eventId);
    if (duplicate)
        return res.status(200).json({ ...duplicate, duplicate: true });
    ...
    const stagedPlateMatches = Boolean(expected
    ...
    if (mayOpen && payload.direction !== 'inbound' && recentRfid?.tags?.length) {
        ...
        const bidirectionalModes = warehouse?.data && typeof warehouse.data === 'object'
            ? warehouse.data.gateBidirectionalModes : undefined;
        ...
        if (!requiresScreenConfirmation && session?.direction === 'out' && session.customer && session.expiresAt.getTime() > Date.now()) {
            ...
            try {
                ...
                for (const tag of autoShippedTags)
                    await db.delete(gatePendingReads).where(and(eq(gatePendingReads.gateNo, gateNo), eq(gatePendingReads.tag, tag)));
            }
            catch (error) {
                mayOpen = false;
    ...
    try {
        ...
        if (decision.rfidTags?.length) {
            ...
            for (const row of resolved.values()) {
                ...
                await db.update(boxes).set({ history, data: { ...row.data, history }, updatedAt: ts }).where(eq(boxes.tag, row.tag));
            }
        }
    }
    catch (error) {
...
lprRouter.delete('/boxes/:tag/events/:eventId', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!row)
        return res.status(404).json({ error: 'not_found', message: 'ไม่พบกล่อง' });
    ...
    if (!removed)
        return res.status(404).json({ error: 'not_found', message: 'ไม่พบประวัติ LPR ที่ต้องการลบ' });
    await db.update(boxes).set({
        history,
        data: { ...row.data, history },
        updatedAt: new Date(),
