...
employeePinRouter.put('/:id/pin', asyncHandler(async (req, res) => {
...
employeePinRouter.post('/:id/pin/verify', asyncHandler(async (req, res) => {
    const { pin } = pinSchema.parse(req.body);
    const db = getDb();
    ...
    if (!row)
        throw httpError(404, 'ไม่พบพนักงาน', 'not_found');
    if (!row.pinHash)
        return res.json({ ok: true, noPinSet: true }); // never set → nothing to check
...
employeePinRouter.post('/:id/pin/reset', pinResetLimiter, asyncHandler(async (req, res) => {
    const db = getDb();
    const rows = await db.select().from(employees).where(eq(employees.id, req.params.id));
    const row = rows[0];
    if (!row)
...
employeePinRouter.post('/:id/pin/reset/verify', asyncHandler(async (req, res) => {
...
employeePinRouter.post('/:id/pin/confirm-reset', asyncHandler(async (req, res) => {
...
employeePinRouter.put('/:id/credentials', asyncHandler(async (req, res) => {
