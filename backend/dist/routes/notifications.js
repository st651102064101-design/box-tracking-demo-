...
notificationsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    if (input.channel === 'email') {
        ...
        if (!email)
            throw httpError(400, 'ลูกค้ารายนี้ยังไม่มีอีเมล กรุณาเพิ่มในข้อมูลลูกค้าก่อน', 'customer_email_missing');
        await sendMail({
            to: email,
    ...
    if (!lineUserId)
        throw httpError(400, 'ลูกค้ารายนี้ยังไม่มี LINE User ID กรุณาเพิ่มในข้อมูลลูกค้าก่อน', 'line_user_id_missing');
    if (!LINE_MESSAGING_USER_ID.test(lineUserId))
        throw httpError(400, 'LINE User ID ต้องเป็นค่าจาก webhook ที่ขึ้นต้นด้วย U และมี 33 ตัวอักษร ไม่ใช่ LINE ID หรือ username', 'line_user_id_invalid');
