...
import { loginSchema, registerSchema, updateRoleSchema, firstSetupSchema, linkEmployeeSchema, forgotPasswordRequestSchema, resetPasswordSchema, } from '../validators/schemas.js';
...
const ACCESS_BY_ROLE = { admin: 'admin', staff: 'operator', viewer: 'viewer' };
 ...
 *  normalized here. */
const ROLE_BY_ACCESS = { admin: 'admin', operator: 'staff', supervisor: 'staff', viewer: 'viewer' };
/**
 ...
 */
authRouter.post('/pda-session', asyncHandler(async (req, res) => {
    const body = (req.body ?? {});
    ...
    if (!identifier && !password) {
        ...
        if (!privateLan)
            throw httpError(403, 'PDA QR session อนุญาตเฉพาะเครือข่ายภายใน', 'pda_lan_only');
...
authRouter.post('/first-setup', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!Number.isInteger(actorId))
        throw httpError(401, 'เซสชันไม่ถูกต้อง', 'invalid_session');
    const [row] = await db.select().from(users).where(eq(users.id, actorId));
    if (!row || !row.mustChangePassword)
        throw httpError(409, 'บัญชีนี้ตั้งค่าเริ่มต้นเรียบร้อยแล้ว', 'first_setup_not_required');
    const [existing] = await db.select().from(employees).where(eq(employees.id, input.username.toUpperCase()));
    if (existing && existing.userId !== row.id)
        throw httpError(409, 'รหัสพนักงานนี้ถูกใช้งานแล้ว', 'employee_taken');
    ...
    if (existing)
        await db.update(employees).set({ name: input.name, userId: row.id, roleId: 1, data, updatedAt: new Date() }).where(eq(employees.id, existing.id));
    else
        await db.insert(employees).values({ id: input.username.toUpperCase(), name: input.name, userId: row.id, roleId: 1, data });
...
authRouter.post('/register', asyncHandler(async (req, res) => {
    ...
    if (existing.length)
        throw httpError(409, 'ชื่อผู้ใช้นี้ถูกใช้แล้ว', 'username_taken');
    const passwordHash = await hashPassword(input.password);
    const [row] = await db
        .insert(users)
...
authRouter.post('/forgot-password', forgotPasswordLimiter, asyncHandler(async (req, res) => {
    ...
    const account = userRow
        ? { kind: 'user', id: userRow.id, email: userRow.email }
        : await (async () => {
            ...
            if (!empRow)
                return null;
            const email = empRow.data?.email?.trim() || null;
            return { kind: 'employee', id: empRow.id, email };
        })();
    // Don't reveal whether the username exists — just claim success either way.
    if (!account)
        return res.json({ ok: true, sentTo: null });
    if (!account.email) {
        throw httpError(400, 'บัญชีนี้ยังไม่มีอีเมลผูกไว้ — ติดต่อผู้ดูแลระบบเพื่อเพิ่มอีเมลก่อน', 'no_email_on_file');
    }
    const otp = genOtp();
...
authRouter.post('/reset-password', forgotPasswordLimiter, asyncHandler(async (req, res) => {
    ...
    const [updatedEmp] = await db
        ...
        .returning();
    const access = updatedEmp.data?.access;
    const accessLevel = typeof access === 'string' && access ? access : 'operator';
    const role = ROLE_BY_ACCESS[accessLevel] ?? 'staff';
    const token = signToken({
        sub: updatedEmp.id,
...
authRouter.get('/users', requireAuth, asyncHandler(async (req, res) => {
    const db = getDb();
...
authRouter.patch('/users/:id/role', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isInteger(id))
        throw httpError(400, 'รหัสผู้ใช้ไม่ถูกต้อง', 'invalid_id');
    const input = updateRoleSchema.parse(req.body);
    const db = getDb();
    const row = await db.transaction(async (tx) => {
        const [before] = await tx.select().from(users).where(eq(users.id, id));
        if (!before)
            throw httpError(404, 'ไม่พบผู้ใช้', 'not_found');
        ...
        if (linkedEmp) {
            ...
            const nextData = { ...linkedEmp.data, access: nextAccess };
...
authRouter.patch('/employees/:empId/link', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
