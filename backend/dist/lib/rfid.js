...
export const EPC_BITS = { EPC_96: 96, EPC_128: 128 };
 ...
 */
export function encodeBarcodeToEpcHex(barcode, bits = EPC_BITS.EPC_96) {
    if (!barcode)
        throw new EpcEncodeError('บาร์โค้ดว่างเปล่า');
    ...
    if (raw.length > hexLen) {
        throw new EpcEncodeError(`บาร์โค้ด "${barcode}" ยาวเกินไปสำหรับแท็ก ${bits} บิต (สูงสุด ${hexLen / 2} ตัวอักษร)`);
    }
 ...
 */
export function decodeEpcHexToBarcode(epcHex) {
    ...
    let i = 0;
    while (i < hex.length - 1 && hex.slice(i, i + 2) === '00')
        i += 2;
...
export function looksLikeHex(code) {
