...
export async function getSuperAdminRole(db) {
...
export async function activeSuperAdminHolders(db) {
    ...
    const employeeIds = empRows
        .filter((r) => (r.data?.status ?? 'active') === 'active')
        .map((r) => r.id);
...
export function totalHolders(h) {
    return h.employeeIds.length + h.userIds.length;
}
//# sourceMappingURL=superAdminGuard.js.map