 ...
 */
export async function resolveBoxesByCodes(db, codes) {
    ...
    const rows = lookup.length
        ? await db
            .select()
            .from(boxes)
            .where(or(inArray(boxes.tag, lookup), inArray(boxes.rfidEpc, lookup), inArray(boxes.rfidTid, lookup)))
        : [];
...
export async function resolveBoxByCode(db, code) {
 ...
 */
export async function associateTag(db, input) {
    ...
    return db.transaction(async (tx) => {
        ...
        const data = { ...box.data, rfidTid, rfidEpc };
        await tx
            .update(boxes)
...
export async function detachTag(db, tag, actor) {
    return db.transaction(async (tx) => {
        const [box] = await tx.select().from(boxes).where(eq(boxes.tag, tag));
        if (!box)
            throw httpError(404, 'ไม่พบกล่อง', 'box_not_found');
