 ...
 */
async function resolveOperator(db, employeeId, recorder) {
    if (!employeeId)
        return { employeeId: '', recorder: recorder ?? 'api' };
    const [row] = await db.select().from(employees).where(eq(employees.id, employeeId));
    if (!row)
        throw httpError(404, `ไม่พบพนักงานรหัส ${employeeId}`, 'employee_not_found');
    const data = (row.data ?? {});
    const status = typeof data.status === 'string' ? data.status : 'active';
    if (status !== 'active') {
        throw httpError(403, `${row.name ?? employeeId} ไม่อยู่ในสถานะปฏิบัติงาน — ติดต่อหัวหน้างาน`, 'employee_inactive');
    }
    return { employeeId: row.id, recorder: row.name ?? recorder ?? employeeId };
}
export async function gateOut(db, input) {
    ...
    if (!cust)
        throw httpError(404, 'ไม่พบลูกค้า', 'customer_not_found');
    const [cfg] = await db.select().from(config);
    const { resolved, missing } = await resolveBoxesByCodes(db, tags);
    if (missing.length)
        throw httpError(404, `ไม่พบกล่อง: ${missing.join(', ')}`, 'box_not_found');
