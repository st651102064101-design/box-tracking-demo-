...
import { boxes, customers, boxTypes, warehouses, gates, gateWebhookStatus, gatePendingReads, gatePrefs, uiPrefs, locations, employees, vehicles, doRecords, putaway, inventory, cycleCounts, events, auditLog, lineLinkInvites, lineNotificationDeliveries, config, sequences, roles, } from '../db/schema.js';
...
export async function clearMovementHistory(db) {
 ...
 * system.
 */
export async function wipeSystem(db) {
    return db.transaction(async (tx) => {
        await tx.execute(sql `select pg_advisory_xact_lock(${STATE_WRITE_LOCK})`);
...
//# sourceMappingURL=systemWipe.js.map