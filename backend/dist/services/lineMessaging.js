...
async function lineRequest(path, body) {
    ...
    try {
        ...
        if (!response.ok)
            console.error('[line-reply]', { status: response.status, body: await response.text() });
    }
    catch (error) {
...
export function replyLineText({ replyToken, text }) {
...
export async function pushLineText({ to, text, retryKey }) {
...
export async function pushLineFlex({ to, altText, contents, retryKey }) {
...
async function pushLineMessages(to, messages, retryKey) {
    ...
    if (!response.ok) {
        ...
        throw httpError(response.status === 400 ? 400 : 502, response.status === 400 ? 'LINE ID หรือข้อความไม่ถูกต้อง' : 'LINE ไม่สามารถรับข้อความได้ในขณะนี้', 'line_push_failed');
    }
    return requestId;
}
//# sourceMappingURL=lineMessaging.js.map