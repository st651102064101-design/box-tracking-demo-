...
const asStrings = (v: unknown): string[] => (Array.isArray(v) ? (v as unknown[]).map(String) : []);

/** Shape returned to clients — the raw row plus the derived "what's missing". */
function present(row: typeof cycleCounts.$inferSelect) {
...
async function loadOr404(id: string) {
  ...
  if (!row) throw httpError(404, 'ไม่พบรอบตรวจนับ', 'cycle_count_not_found');
  return row;
}
...
cycleCountsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const conds = [];
    if (typeof req.query.status === 'string') conds.push(eq(cycleCounts.status, req.query.status));
    if (typeof req.query.wh === 'string') conds.push(eq(cycleCounts.wh, req.query.wh));
    const rows = await db
      .select()
      .from(cycleCounts)
...
cycleCountsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [open] = await db
      ...
      .where(
        and(
          eq(cycleCounts.wh, input.wh),
          eq(cycleCounts.zone, input.zone),
          eq(cycleCounts.status, 'open'),
        ),
      );
    if (open) return res.status(200).json({ ...present(open), resumed: true });
...
cycleCountsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const box of resolved.values()) {
      if (expected.has(box.tag)) counted.add(box.tag);
      else unexpected.add(box.tag);
    }
...
cycleCountsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!result.missing.length) return res.json({ updated: 0, tags: [] });
