...
const files = {forklift:'forklift.glb',door:'fire_exit_door.glb',exit:'emergency_exit_low_poly.glb',pallet:'wooden_pallets.glb'};
const cache = new Map();
export async function convertAsset(type, T) {
  ...
  const scale = type==='forklift' ? 325/Math.max(size.x,size.z) : type==='door' ? 210/size.y : type==='exit' ? 60/size.x : 120/Math.max(size.x,size.z);
