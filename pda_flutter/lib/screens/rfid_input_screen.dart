...
class _Read {
  ...
  factory _Read.fromTagRead(RfidTagRead r, DateTime at, {int count = 1}) =>
      _Read(
        r.epc,
        at,
        tid: r.tid,
...
class _RfidInputScreenState extends State<RfidInputScreen> {
  ...
  void initState() {
    ...
    _triggerSub =
        rfid.triggers.listen((pressed) => setState(() => _reading = pressed));
  ...
  /// A barcode from the imager, listed alongside the RFID reads so this
  /// screen can answer "what does the scanner actually send" for both
  /// channels. There is no typed-code path any more — a made-up code proves
  /// nothing about the hardware, which is the only thing this screen is for.
  void _addScanned(String v) {
    v = v.trim();
    if (v.isEmpty) return;
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return ScanCapture(
      ...
      child: AutoHideHeader(
        header: StickyHeader(
            onBack: c.backToHome, title: const Text('รับค่า RFID')),
        body: Column(
          children: [
            Expanded(
              child: ListView(
                ...
                children: [
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: FilledButton.icon(
                            onPressed:
                                c.rfid.supported ? () => _toggleRead(c) : null,
                            icon: Icon(
                                _reading ? Icons.stop : Icons.wifi_tethering),
                            label:
                                Text(_reading ? 'หยุดอ่าน' : 'เริ่มอ่าน RFID'),
                  ...
                  if (_reads.isEmpty)
                    Padding(
                      ...
                      child: Center(
                        child: Text(
                            'ยังไม่มีข้อมูล — กด "เริ่มอ่าน RFID" แล้วยิงแท็ก',
                              ...
                              children: [
                                ...
                                Wrap(
                                  ...
                                  children: [
                                    _field('TID', r.tid),
                                    _field('PC',
                                        r.pc?.toRadixString(16).toUpperCase()),
                                    _field('CRC', r.crc),
